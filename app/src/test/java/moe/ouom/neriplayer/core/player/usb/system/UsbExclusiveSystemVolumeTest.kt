...
class UsbExclusiveSystemVolumeTest {
    ...
    @Test
    fun `system media volume uses linear gain`() {
        assertEquals(0f, usbExclusiveExponentialSystemVolumeGain(0f), 0.0001f)
        assertEquals(0.5f, usbExclusiveExponentialSystemVolumeGain(0.5f), 0.0001f)
    ...
    fun `native volume combines player and system gain safely`() {
        assertEquals(0.25f, usbExclusiveEffectiveNativeVolume(0.5f, 0.5f), 0.0001f)
